...
export default function ChangeCalculator({ total, onConfirm, onCancel }: ChangeCalculatorProps) {
  ...
  const getAmountPaid = (): number => {
    ...
    // Si no ingresó nada, asumir pago exacto
    return total;
  };
  ...
  const handleConfirm = () => {
    // SIEMPRE permite confirmar, incluso sin seleccionar nada
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Vuelto - Solo se muestra si hay algo seleccionado/ingresado */}
      {(selectedBill || manualAmount) && amountPaid > 0 && (
        <div className={cn(
      ...
      {!selectedBill && !manualAmount && (
        <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
          <div className="text-sm font-medium text-blue-700 mb-1">
            Pago exacto
          </div>
          <div className="text-xs text-blue-600">
            Si no ingresas un monto, se asumirá que el cliente pagó exacto
          </div>
        </div>
      )}

      {/* Botones - CONFIRMAR siempre habilitado */}
